...
func TestJQTemplateFunc(t *testing.T) {
	tests := []struct {
		name                string
		query               string
		input               string
		...
		},
		{
			...
			query:               ".data",
			input:               `{"data": 10}`,
			expectedUnmarshaled: 10,
		},
		{
			name:                "Single Number Value",
			query:               ".data",
			input:               `{"data": 9.3}`,
			expectedUnmarshaled: 9.3,
		},
		{
			name:                "Multiple String Value",
			query:               ".data",
			input:               `{"data": ["A","B"]}`,
			expectedUnmarshaled: []string{"A", "B"},
		},
		{
			name:                "Multiple Integer Value",
			query:               ".data",
			input:               `{"data": [1,2]}`,
			expectedUnmarshaled: []int{1, 2},
		},
		{
			name:                "Multiple Number Value",
			query:               ".data",
			input:               `{"data": [9.2,9.2]}`,
			expectedUnmarshaled: []float64{9.2, 9.2},
		},
		{
			name:                "Multiple Map Values",
			query:               ".data",
			input:               `{"data": [{"foo": "bar"},{"bar": "foo"}]}`,
			expectedUnmarshaled: []map[string]string{{"foo": "bar"}, {"bar": "foo"}},
		},
		{
			name:                "Select Expression",
			query:               `.[] | select(.data == "foo")`,
			input:               `[{"data": "bar"}, {"data": "foo"}]`,
			expectedUnmarshaled: map[string]string{"data": "foo"},
		},
		{
			name:                "Map Expression",
			query:               `map(.[1] as $needle | .[0] | contains($needle))`,
			input:               `[[["foobar", "foobaz"], ["baz", "bar"]], [["foobar", "foobaz"], ["foo"]], [["foobar", "foobaz"], ["blap"]]]`,
			expectedUnmarshaled: []bool{true, true, false},
		},
		{
			name:                "Builtin base64 Function",
			query:               `.data | @base64`,
			input:               `{"data": "data"}`,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			ret, err := JQ(test.query, test.input)
			assert.NoError(t, err)

			var expectedData string
			switch d := test.expectedUnmarshaled.(type) {
			case string:
				expectedData = d
			default:
			...
			// execute in text template
			tpl, err := template.New("").Funcs(fm).Parse(fmt.Sprintf(`{{- . | jq %s -}}{{- . | toBytes | jqBytes %s -}}`, strconv.Quote(test.query), strconv.Quote(test.query)))
			assert.NoError(t, err)

			buf := new(bytes.Buffer)
			err = tpl.Execute(buf, test.input)
			assert.NoError(t, err)

			assert.Equal(t, expectedData+expectedData, buf.String())
		})
	}
}

func TestJQCompability(t *testing.T) {
	s := bufio.NewScanner(bytes.NewReader([]byte(jqTestData)))
	...
	for s.Scan() {
		...
		input = s.Text()
		s.Scan()
		expectedResult = strings.TrimSuffix(strings.TrimPrefix(s.Text(), `"`), `"`)

		ret, err := JQ(query, input)
		assert.NoError(t, err)

		if strings.HasPrefix(expectedResult, "[") || strings.HasPrefix(expectedResult, "{") {
			assert.JSONEq(t, expectedResult, ret, "line %d: %q failed, input = %q", line-3, query, input)
		} else {
			assert.Equal(t, expectedResult, ret, "line %d: %q failed, input = %q", line-3, query, input)
		}
	}
}

const jqTestData = `
# https://raw.githubusercontent.com/stedolan/jq/master/tests/jq.test
